...
# Global Layout Styling: Monopoly Cruise Board Theme
st.set_page_config(page_title="Cruise Boardroom Monopoly", layout="wide")

st.markdown("""
    <style>
    /* Dark Nautical Board Game Aesthetic Layout */
    .stApp {
        background-color: #0a192f !important;
        background-image: radial-gradient(rgba(16, 185, 129, 0.1) 2px, transparent 0) !important;
        background-size: 30px 30px !important;
    }
    
    /* Monopoly Style Property & Board Card Container UI */
    .monopoly-card {
        background-color: #112240 !important;
        border: 2px solid #10b981 !important;
        border-top: 14px solid #f59e0b !important;
    ...
    /* Uniform Global Typography Rules Override */
...
if st.session_state.phase == 0:
    st.markdown("<h1>🚢 CRUISE SHIP OPERATIONS MONOPOLY</h1>", unsafe_allow_html=True)
    st.write("Welcome corporate cruise manager. Select your group parameter to initiate asset deeds registry.")
    
    group_choice = st.selectbox("Select Your Board Group Number (1-8):", list(GROUP_DATA.keys()))
    cfg = GROUP_DATA[group_choice]
    
    # Requirement 1: Removed box/deed container text inputs underneath group selector
...
# --- PHASE 1 - 5: THE INTERACTIVE PROPERTY EVENT GRID PLATFORM ---
    ...
    st.markdown(f"<h2>📊 Assets Scoreboard | {st.session_state.group}</h2>", unsafe_allow_html=True)
    ...
    with m_col4: st.metric("💀 Total Casualties Logged", f"{st.session_state.dead} Dead")
    st.markdown("---")
    
    # 2. Live Profile Info Header
    st.markdown(f"""
        <div class="monopoly-card" style="border-top: 6px solid #10b981 !important; margin-bottom:10px; padding:15px !important;">
            <b>🚢 Vessel ID:</b> <span class="gold-text">{st.session_state.v_id}</span> | 
            <b>Cruise Line:</b> {st.session_state.brand} | 
            <b>Attraction Focus:</b> {st.session_state.theme} | 
            <b>Route:</b> {st.session_state.route} | 
            <b>Demographics:</b> {st.session_state.market} Market
        </div>
    """, unsafe_allow_html=True)

    # 3. Core Decisions Engine Logic Blocks
    st.markdown(f"<h3>🎲 Monopoly Card Round: {st.session_state.phase} / 5</h3>", unsafe_allow_html=True)
    
    # Static scenario database assignments to avoid compilation parsing blocks
    if st.session_state.phase == 1:
        ...
        title, emoji = "AUDIT CRISIS: VIP Whales Fraud Accusations", "🎰"
